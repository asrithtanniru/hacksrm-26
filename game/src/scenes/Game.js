...
export class Game extends Scene {
  ...
  init(data) {
    ...
    this.availableCharacterTokens = new Set(availableCharacters.map((item) => item?.character_token).filter(Boolean))
  ...
  setupDataHavenControls() {
    this.input.keyboard.on('keydown-U', () => {
      this.handleAgentMemoryUpload()
    })
  }

  async handleAgentMemoryUpload() {
    if (!this.activeNearbyCharacterId) {
      this.showToast('No agent nearby to sync memory with.')
      return
    }

    // Get basic local info
    const agentId = this.activeNearbyCharacterId
    const localChar = this.characters.find((c) => c.id === agentId)
    const agentName = localChar ? localChar.name : agentId
    // 'backendToken' holds the ID used in token_server/agent_info.json
    const backendToken = localChar ? localChar.backendToken : null

    this.showToast(`Syncing memory for ${agentName}...`)

    try {
      // 1. Fetch REAL dynamic data from backend
      let memoryData = {}
      try {
        const response = await ApiService.listLivekitCharacters()
        const allChars = response.characters || []
        // Find by token if possible, else by name
        const remoteChar = allChars.find((c) => c.character_token === backendToken || c.character_token === agentId)

        if (remoteChar) {
          // Use real backend data
          memoryData = {
            agentId: remoteChar.character_token,
            agentName: remoteChar.name,
            role: remoteChar.role || 'Unknown role',
            emotion: remoteChar.emotion_tag || 'Neutral',
            timestamp: Date.now(),
            source: 'Game Client Upload',
          }
        } else {
          // Fallback if backend doesn't have it listed
          memoryData = {
            agentId: agentId,
            agentName: agentName,
        ...
        console.warn('Failed to fetch rich agent info, using local fallback', e)
        memoryData = {
          agentId: agentId,
          agentName: agentName,
      ...
      this.showToast('Upload failed on DataHaven. Check console.')
    }
  }
  ...
  createCharacters(layers) {
    const characterConfigs = [
      {
        ...
        proximityScript: ['Mira: "If you can hear me, the protocol still marks you as a subject."', '"Stay calm. I will guide you through the archive."'],
      ...
      },
      {
        ...
        proximityScript: ['Raghav stares at the wall. Symbols shift when you blink.', '"Room 204 remembers everything."'],
      ...
      },
      {
        ...
        proximityScript: ['Meera whispers from the corner, tears glitching into black streaks.', '"Please... do not leave me in this loop again."'],
      ...
      },
      {
        ...
        proximityScript: ['Metal scraping echoes before the Janitor appears beside you.', '"You are early, Subject. The building already knows your fear."'],
      ...
      },
      {
        ...
        proximityScript: ['The PA crackles. A synthetic voice overlays your heartbeat.', `"Subject ${this.playerName}: preservation pathway available."`],
      ...
      },
      {
        ...
        proximityScript: ['Archive Nurse checks a broken slate of patient records.', '"Most files are corrupted, but some memories are still recoverable."'],
      ...
      },
      {
        ...
        proximityScript: ['Nila speaks as if finishing a sentence from another timeline.', '"The core keeps replaying futures that never happened."'],
      ...
      },
      {
        ...
        proximityScript: ['Kabir traces route markers on the floor tiles.', '"Room 14-B still stores neural backups. Take the red corridor carefully."'],
      ...
      },
      {
        ...
        proximityScript: ['Omkar keeps his eyes on the surveillance lamps.', '"Bodies stayed. Minds were archived. That was the real transfer."'],
      ...
      },
      {
        ...
        proximityScript: ['A containment terminal projects a humanoid guard silhouette.', '"Authorization incomplete. Escalate to core protocol."'],
      ...
      },
      {
        ...
        proximityScript: ['A child-shaped echo flickers at the edge of the corridor light.', '"I am what stayed after the scream was archived."'],
    ...
    characterConfigs.forEach((config) => {
      ...
      if (this.availableCharacterTokens.size > 0 && !this.availableCharacterTokens.has(preferredToken) && this.fallbackCharacterToken) {
    ...
    for (let i = 0; i < this.characters.length; i++) {
      for (let j = i + 1; j < this.characters.length; j++) {
        this.physics.add.collider(this.characters[i].sprite, this.characters[j].sprite)
      }
    }
  }
  ...
  createProximityDialogue() {
    ...
    const hintText = this.add
      .text(boxX + boxWidth - 12, boxY + boxHeight - 8, 'Press A to talk', {
        font: gameFont(12),
        color: '#ffd28f',
      })
      .setOrigin(1, 1)
  ...
  showProximityDialogue(character) {
    ...
    const scriptLines = character.proximityScript.length > 0 ? character.proximityScript : [character.defaultMessage]
  ...
  setupPlayer(map, worldLayer) {
    ...
    this.player = this.physics.add.sprite(spawnPoint.x, spawnPoint.y, 'sophia', 'sophia-front').setSize(30, 40).setOffset(0, 6)
  ...
  updatePlayerNameLabelPosition() {
    ...
    this.playerNameLabel.setPosition(this.player.x, this.player.y - this.player.height / 2 - 10)
  }
  ...
  createTopBarUi() {
    ...
    const disconnectBtn = this.add
      ...
      .setInteractive({ useHandCursor: true })
    const menuMainBtn = this.add
      .text(36, 112, 'Main Menu', {
        font: gameFont(13),
        color: '#ffffff',
        backgroundColor: '#1d3651',
        padding: { x: 8, y: 5 },
